...
## [判断三角形](https://leetcode.cn/problems/triangle-judgement/description/)
...
```mysql
-- 判断三个数是否能成为三角形的标准是"任意两边之和大于第三边，任意两边之差小于第三边"
select x, y, z,
	case 
...
## [只出现一次的最大数字](https://leetcode.cn/problems/biggest-single-number/description/)
...
## [有趣的电影](https://leetcode.cn/problems/not-boring-movies/description/)
...
## [换座位](https://leetcode.cn/problems/exchange-seats/description/)
...
```mysql
# 题目可以理解成座位号为奇数的学生的id要+1，为偶数的学生要-1
select (
	case
...
from Seat
## 如果学生人数为奇数，最后一个学生学号不变
# 我们多查一个表，这个表只有一个count字段，这个字段是这个表的总数
# 如果学生学号等于这个count字段，那么就代表他是最后一个学生
select （
	case
